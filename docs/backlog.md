...
- **Eyeball `.npmrc` once** (1.1 security audit). The secrets-fence hook blocks agents from reading it by design; the expected content is scaffold defaults only. A one-time human check closes the loop.
