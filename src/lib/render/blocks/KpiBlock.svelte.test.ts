...
describe('KpiBlock render', () => {
		...
		const { container } = render(KpiBlock, {
			block: block({
				binding: {
					...
					fields: [{ name: 'incidents', type: 'number' }]
				}
			})
		});
	...
	it('omits the data-as-of caption when the block is not data-bound (static items)', () => {
	...
	it('omits the data-as-of caption when the binding carries no timestamp', () => {
		const { container } = render(KpiBlock, {
			block: block({
				binding: { dataSetId: 'ds-1', fields: [{ name: 'incidents', type: 'number' }] }
			})
		});
		expect(container.querySelector('.data-as-of')).toBeNull();
	});
